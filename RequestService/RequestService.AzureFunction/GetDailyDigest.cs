...
{
    public class GetDailyDigest
    {

        private readonly IDailyDigestService _dailyDigestService;
        public GetDailyDigest(IDailyDigestService dailyDigestService)
        {
            _dailyDigestService = dailyDigestService;
        }

        [FunctionName("GetDailyDigest")]
        public async Task Run([TimerTrigger("%DailyDigestEmailTimer%")] TimerInfo timerInfo, ILogger log, CancellationToken cancellationToken)
        {
            try
            {
